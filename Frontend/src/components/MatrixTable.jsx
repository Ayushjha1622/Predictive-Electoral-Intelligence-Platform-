        ...
        <table className="w-full text-[10px] font-bold uppercase tracking-widest text-left">
          <thead>
            <tr className="bg-slate-900/50">
              <th className="py-4 px-6 text-slate-600 border-r border-white/5">Factor_ID</th>
              {data.map(c => (
                <th key={c.name} className="py-4 px-6 text-center text-white border-r border-white/5 last:border-0">{c.name}</th>
              ))}
            </tr>
          </thead>
          ...
          <tbody>
            {factors.map((f, idx) => (
              <tr key={f.key} className={`${idx % 2 === 0 ? "bg-white/[0.02]" : "bg-transparent"} hover:bg-white/[0.05] transition-colors`}>
                <td className="py-3 px-6 text-slate-500 border-r border-white/5">{f.label}</td>
                {data.map(c => (
                  <td key={c.name} className="py-3 px-6 text-center border-r border-white/5 last:border-0">
            ...
            <tr className="bg-cyan-400/[0.05] border-t border-cyan-400/20">
              <td className="py-4 px-6 text-cyan-400 font-black">POWER_INDEX</td>
              {data.map(c => (
                <td key={c.name} className="py-4 px-6 text-center text-cyan-400 font-black text-xs border-r border-cyan-400/10 last:border-0">
                  {c.score?.toFixed(2)}
                </td>
              ))}
            </tr>
